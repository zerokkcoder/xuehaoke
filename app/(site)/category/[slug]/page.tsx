    ...
    const loadCategory = async () => {
      try {
        ...
        if (res.ok && json?.success) {
          ...
          const found = list.find((c: any) => String(c.slug || '') === categorySlug)
            ...
            {category.subcategories && category.subcategories.length > 0 && (
              <>
                {category.subcategories.map((subcategory: any) => (
                  <Link key={subcategory.id} href={`/category/${category?.slug}/${subcategory.slug}`} className="px-2 py-0.5 rounded-full text黑">
                    {subcategory.name}
                  </Link>
                ))}
              </>
