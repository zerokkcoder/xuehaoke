...
export default function CategoryDetailClient({
  ...
  const loadMoreResources = async (sortOverride?: 'latest' | 'downloads' | 'views', pageOverride?: number, force?: boolean) => {
    ...
    try {
      ...
      const url = `/api/resources?page=${requestedPage}&size=${size}&categorySlug=${encodeURIComponent(categorySlug)}&sort=${activeSort}`
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
          ...
          <div className="relative w-full h-48 md:h-64 overflow-hidden card">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center text-white">
                <h1 className="text-2xl md:text-3xl font-bold mb-2">{initialCategory.name}</h1>
        ...
        <div className="card p-3 text-sm mt-4">
          <div className="flex items-center flex-wrap gap-2 mb-2">
            ...
            <span className="px-2 py-0.5 rounded-full bg-violet-500 text-white cursor-default">全部</span>
            {initialCategory.subcategories && initialCategory.subcategories.length > 0 && (
              <>
                {initialCategory.subcategories.map((subcategory) => (
                  <Link key={subcategory.id} href={`/category/${categorySlug}/${subcategory.slug}`} className="px-2 py-0.5 rounded-full text-foreground hover:bg-muted transition-colors">
                    {subcategory.name}
                  </Link>
                ))}
              </>
